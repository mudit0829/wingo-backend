...
// ✅ Get Latest Rounds (Filtered by gameType)
...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    let totalUpcoming = await Round.countDocuments(filter).maxTimeMS(2000);

    // If no future rounds → create placeholders
    if (upcomingRounds.length === 0) {
      ...
      for (let i = 0; i < pageSize; i++) {
        ...
        upcomingRounds.push({ roundId, startTime: futureTime, gameType, resultNumber: null, resultColor: null });
      }
      totalUpcoming = 1000;
    }
...
router.get('/current', async (req, res) => {
  try {
    ...
    res.json({ ...round, duration, endTime });
  } catch (err) {
